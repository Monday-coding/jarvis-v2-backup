...
class PostgresVectorMemoryArchitecture:
    ...
    def __init__(self):
        ...
        self.memory_types = {
            'short_term': {
                'description': '短期記憶（最近 10 條消息）',
                'storage': 'PostgreSQL（JSONB）',
                'retention': 'Session 結束',
                'vector': False
            },
            'long_term': {
                'description': '長期記憶（最近 100 條消息）',
                'storage': 'PostgreSQL + pgvector（VECTOR(1536)）',
                'retention': '跨 Sessions',
                'vector': True
            },
            'user_preferences': {
                'description': '用戶偏好（自定義設定）',
                'storage': 'PostgreSQL（JSONB）',
                'retention': '永久',
                'vector': False
            },
            'system_state': {
                'description': '系統狀態（系統配置、Agent 狀態等）',
                'storage': 'PostgreSQL（JSONB）',
                'retention': '永久',
                'vector': False
            }
    ...
    def check_pgvector_installation(self):
        """檢查 pgvector 安裝"""
        print("=" * 60)
        print("檢查 pgvector 安裝")
        print("=" * 60)
        print()
        
        try:
            # 檢查 Docker 容器中是否已安裝 pgvector
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                if result.returncode == 0:
                    ...
                    # 啟用擴展
                    result = subprocess.run(
                        ['docker', 'exec', self.docker_container, 'psql', '-U', self.db_user, '-d', self.db_name, '-c', "CREATE EXTENSION vector"],
                        capture_output=True,
                        text=True,
                        timeout=30
                    )
    ...
    def create_memory_tables(self):
        ...
        try:
            ...
            subprocess.run(
                ['docker', 'exec', self.docker_container, 'psql', '-U', self.db_user, '-d', self.db_name, '-c', "CREATE INDEX IF NOT EXISTS idx_long_term_embedding ON long_term_memory USING ivfflat (message_embedding vector_cosine_ops)"],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def test_memory_operations(self):
        ...
        try:
            ...
            result = subprocess.run(
                ['docker', 'exec', self.docker_container, 'psql', '-U', self.db_user, '-d', self.db_name, '-c', "SELECT * FROM search_long_term_memory('conv_001', '[0.1,0.2,0.3]', 5)"],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def research_memory_architecture(self):
        """研究記憶架構"""
        print("=" * 60)
        print("PostgreSQL + pgvector 記憶架構研究")
        print("=" * 60)
        print()
        
        # 1. 檢查 pgvector 安裝
        print("[第 1 步] 檢查 pgvector 安裝...")
        pgvector_installed = self.check_pgvector_installation()
        print()
        ...
        print()
        print("記憶類型：")
        print("  1. 短期記憶（short_term）")
        print("     - 儲存：PostgreSQL（JSONB）")
        print("     - 保留：Session 結束")
        print("     - 用途：最近 10 條消息，快速訪問")
        print()
        print("  2. 長期記憶（long_term）")
        print("     - 儲存：PostgreSQL + pgvector（VECTOR(1536)）")
        print("     - 保留：跨 Sessions")
        print("     - 用途：最近 100 條消息，向量檢索")
        print()
        print("  3. 用戶偏好（user_preferences）")
        print("     - 儲存：PostgreSQL（JSONB）")
        print("     - 保留：永久")
        print("     - 用途：自定義設置、閾值、偏好")
        print()
        print("  4. 系統狀態（system_state）")
        print("     - 儲存：PostgreSQL（JSONB）")
        print("     - 保留：永久")
        print("     - 用途：系統配置、Agent 狀態等")
        print()
        print("向量檢索：")
        print("  - 儲存格式：VECTOR(1536)")
        ...
        print("  2. 長期記憶：使用 VECTOR(1536)，向量檢索")
...
def main():
    """主函數 - 研究和實現記憶架構"""
    print("PostgreSQL + pgvector 記憶架構研究和實現")
    print("=" * 60)
    print()
    
    # 創建架構研究器
    architecture = PostgresVectorMemoryArchitecture()
    
    # 研究記憶架構
    architecture.research_memory_architecture()
