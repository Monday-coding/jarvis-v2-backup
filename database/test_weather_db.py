...
def test_database_operations():
    ...
    try:
        with db:
            ...
            print(f"  找到 {len(weather_data)} 條天氣數據")
            for data in weather_data:
                print(f"    - {data['observation_time']}: {data['temperature']}度, {data['rainfall']}mm")
            print()
            ...
            print(f"  找到 {len(alerts)} 條警告")
            for alert in alerts:
                print(f"    - {alert['alert_type']} ({alert['severity']}): {alert['title']}")
